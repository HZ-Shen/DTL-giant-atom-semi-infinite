...
omegax=1.2*pi/tau;k=3;
omegal=0.7*pi;Omega_3ls=-0.5*pi;Gamma=0.3*pi/tau; 
omegae=2*Omega_3ls^2/(Gamma*(N+1)*cot(k*pi/(N+1))-4*k*pi/((N+1)*tau)+2*omegax)-omegal+2*k*pi/((N+1)*tau);%omegae has expression                                                    
...
for j=1:length(Cx)-1
    ...
    for m=1:N
        for n=1:N                                                          
            ...
            HH1=exp(1i*omegal/2*abs(m-n)/v);HH2=exp(1i*omegal/2*(m+n)/v);
...
ylim(axes2,[0 0.4])
set(axes2,'FontName','Times New Roman','FontSize',14,'Layer','top',...
...
title('(d)','position',[0.5,0.3],'FontSize',14);