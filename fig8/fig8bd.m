...
omegax=Omega_3ls^2/(omegae+omegal-2*k1*pi/((N+1)*tau))-cot(k1*pi/(N+1))*(Gamma*tau)*(N+1)/2+2*k1*pi/((N+1)*tau);
    ...
    for m=1:N
        for n=1:N                                                          
            ...
            elseif tlist(j)-(m+n)*tau>0                                    %t>(m+n)*tau
                DeltaCx = DeltaCx - (Gamma)/2*HH1^tau*Cx(index1)+ Gamma/2*HH2^tau*Cx(index2);
...
omegak1=2*k1*pi/((N+1)*tau)-omegal/2; 
omegak2=2*k2*pi/((N+1)*tau)-omegal/2; 
S_k1=-1i*omegak1;
S_k2=-1i*omegak2; 
for i=1:length(tlist)
    t=tlist(i);
    ce(i)=(Ce(1)*(S_k1+1i*Delta1+0.5*Gamma*(2*(N+1)/(1-exp(1i*2*k1*pi/(N+1)))-(N+1)))-1i*Omega_3ls*Cx(1))*exp(S_k1*t)...
        /((1+Gamma/2*((N+1)*tau/((sin(k1*pi/(N+1)))^2)))*(S_k1+1i*Delta2)+(S_k1+1i*Delta1+Gamma/2*(2*(N+1)/(1-exp(1i*2*k1*pi/(N+1)))-(N+1))))+(Ce(1)*(S_k2+1i*Delta1+0.5*Gamma*(2*(N+1)/(1-exp(1i*2*k2*pi/(N+1)))-(N+1)))-1i*Omega_3ls*Cx(1))*exp(S_k2*t)...
        /((1+Gamma/2*((N+1)*tau/((sin(k2*pi/(N+1)))^2)))*(S_k2+1i*Delta2)+(S_k2+1i*Delta1+Gamma/2*(2*(N+1)/(1-exp(1i*2*k2*pi/(N+1)))-(N+1))));
    
    cx(i)=(Cx(1)*(S_k1+1i*Delta2)-1i*Omega_3ls*Ce(1))*exp(S_k1*t)...
        /((1+Gamma/2*((N+1)*tau/((sin(k1*pi/(N+1)))^2)))*(S_k1+1i*Delta2)+(S_k1+1i*Delta1+Gamma/2*(2*(N+1)/(1-exp(1i*2*k1*pi/(N+1)))-(N+1))))+(Cx(1)*(S_k2+1i*Delta2)-1i*Omega_3ls*Ce(1))*exp(S_k2*t)...
        /((1+Gamma/2*((N+1)*tau/((sin(k2*pi/(N+1)))^2)))*(S_k2+1i*Delta2)+(S_k2+1i*Delta1+Gamma/2*(2*(N+1)/(1-exp(1i*2*k2*pi/(N+1)))-(N+1))));
end 
Tlist=Gamma*tlist;
...
plot1 = plot(Tlist,Y1);
set(plot1(1),'DisplayName','Eq. (8)','LineWidth',1.1,'Color',[69/255 123/255 157/255]);
set(plot1(2),'DisplayName','Eq. (39)','MarkerSize',1.1,'LineWidth',1.1,...
    'LineStyle','-.',...
...
legend('boxoff');
title('(b)','position',[6,0.55],'FontSize',14);
% 
axes2 = axes('Position',[0.787994791666665 0.77972251867663 0.0958999999999986 0.1364]);
hold(axes2,'on');
plot1 = plot(Tlist,Y1);
set(plot1(1),'LineWidth',1.1,'Color',[69/255 123/255 157/255]);
...
subplot('position',[0.60 0.13 0.366471449487555 0.378846153846154])

plot1 = plot(Tlist,Y2);
set(plot1(1),'DisplayName','Eq. (9)','LineWidth',1.1,'Color',[69/255 123/255 157/255]);
set(plot1(2),'DisplayName','Eq. (41)','MarkerSize',1.1,'LineWidth',1.1,...
    'LineStyle','-.',...
    'Color',[230/255 57/255 70/255]);
...
lgd.Interpreter = 'latex';
legend('boxoff');
title('(d)','position',[6,0.64],'FontSize',14);
% 
axes2 = axes('Position',[0.787994791666665 0.31972251867663 0.0958999999999986 0.1364]);
hold(axes2,'on');
plot1 = plot(Tlist,Y2);
set(plot1(1),'LineWidth',1.1,'Color',[69/255 123/255 157/255]);
set(plot1(2),'LineWidth',1.1,...
